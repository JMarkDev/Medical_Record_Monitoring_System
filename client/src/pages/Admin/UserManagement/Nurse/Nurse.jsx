...
const Nurse = () => {
  ...
  const nurse = useSelector(getAllUsers);
  ...
  const fetchUpdate = () => {
    dispatch(fetchUsers(rolesList?.nurse));
  };
  ...
  useEffect(() => {
    if (searchTerm) {
      dispatch(searchNurseRole(searchTerm));
    } else {
      dispatch(fetchUsers(rolesList?.nurse));
    }
  }, [searchTerm, dispatch]);
  ...
  const currentData = nurse?.slice(indexOfFirstDocument, indexOfLastDocument);
